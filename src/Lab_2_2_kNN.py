...
class knn:
    ...
    def fit(self, X_train: np.ndarray, y_train: np.ndarray, k: int = 5, p: int = 2):
        ...
        elif k <= 0 or not isinstance(k, int) or p <= 0 or not isinstance(p, int):
    ...
    def predict(self, X: np.ndarray) -> np.ndarray:
        ...
        for i in X:
            ...
            y_pred = self.most_common_label(knn_labels)
    ...
    def predict_proba(self, X):
        ...
        Args:
            X (np.): data samples to predict their labels.
        ...
        for punto in X:
            ...
            clases_cercanas = self.y_train[indices]
            prob = np.array([np.sum(clases_cercanas == c) / self.k for c in clases])
            probs.append(prob)
    ...
    def compute_distances(self, point: np.ndarray) -> np.ndarray:
        ...
        """
        # TODO
        distances = np.array([minkowski_distance(i, point, self.p) for i in self.x_train])
        return distances
...
def plot_calibration_curve(y_true, y_probs, positive_label, n_bins=10):
    ...
    plt.plot(bin_centers, true_proportions, marker='o', linestyle='-', color='b')
...
def plot_probability_histograms(y_true, y_probs, positive_label, n_bins=10):
    ...
    plt.title('Histogram of Predicted Probabilities for Negative Class')
    plt.xlabel('Predicted Probability')
...
def plot_roc_curve(y_true, y_probs, positive_label):
    ...
    for threshold in thresholds:
        ...
        fpr_th = fp / (fp + tn) if fp + tn != 0 else 0
        tpr_th = tp / (tp + fn) if tp + fn != 0 else 0
        fpr.append(fpr_th)
        tpr.append(tpr_th)
